...
gsed -i "s|@AWS_REGION@|${AWS_REGION}|g" cryosparc-pcluster.config
gsed -i "s|@AWS_KEY@|${AWS_KEY}|g" cryosparc-pcluster.config
gsed -i "s|@VPC_ID@|${VPC_ID}|g" cryosparc-pcluster.config
gsed -i "s|@COMPUTE_SUBNET_ID@|${COMPUTE_SUBNET_ID}|g" cryosparc-pcluster.config
gsed -i "s|@MASTER_SUBNET_ID@|${MASTER_SUBNET_ID}|g" cryosparc-pcluster.config
gsed -i "s|@S3_CONFIG_BUCKET@|${CONFIG_BUCKET_NAME}|g" cryosparc-pcluster.config
gsed -i "s|@S3_DATA_BUCKET@|${DATA_BUCKET_NAME}|g" cryosparc-pcluster.config

# Create ParallelCluster
pcluster create --region ${AWS_REGION} -nr -c cryosparc-pcluster.config ${STACK_NAME}

CRYOSPARC_HEADNODE_IPADDR=$(aws ec2 describe-instances --region $AWS_REGION --filters Name=tag:ClusterName,Values=cryosparc Name=tag:Name,Values=Master --query 'Reservations[].Instances[].NetworkInterfaces[].Association[].PublicIp' --output text)
echo ""
echo ""
...
echo ""
echo "cryosparcm createuser --email "<email_address>" --password "<password>" --username "<username>"   --firstname "<user first name>" --lastname "<user lasname>""
echo ""
echo ""
